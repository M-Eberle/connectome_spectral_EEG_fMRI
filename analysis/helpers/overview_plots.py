...
from helpers.general import corrcoef2D
...
def plot_ex_signal_fMRI_EEG_one(
    ...
    plt.show()
    plt.savefig(
        f"../results/overview_plots/exemplary_signal_participant/per_{mode}/participant_{ex_participant+1}_{mode}_{ex_region_harmonic}.png"
    )
...
def plot_ex_evecs_3D(Gs, ex_participant):
    ...
    plt.show()
    plt.savefig(
        f"../results/overview_plots/signal_on_graph/eigenvectors/participant_{ex_participant}"
    )
